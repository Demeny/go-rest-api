...
const SERVER = "mongodb://bogdan:pass123@ds225382.mlab.com:25382/dummysore"

// DBNAME the name of the DB instance
const DBNAME = "dummysore"
...
// AddUser adds a Product in the DB
func (r Repository) AddUser(user User) bool {
	session, err := mgo.Dial(SERVER)
	defer session.Close()

	session.DB(DBNAME).C(USERS).Insert(user)
	if err != nil {
		log.Fatal(err)
		return false
	}
